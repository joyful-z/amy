...
async def _main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="amy-computer-v2-") as temp_dir:
        ...
        try:
            ...
            appended = await runtime.type(" Amy", element_ref=editor.ref)
            assert appended.verification_status is VerificationStatus.UNVERIFIED
            _, editor = await _observe_until_text(runtime, "hello Amy")
            assert editor.value == "hello Amy"
            print("PASS: computer_type 保留原内容并得到 hello Amy")
