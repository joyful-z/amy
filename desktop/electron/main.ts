...
ipcMain.handle('amy:open-external', async (_event, url: unknown) => {
  if (typeof url !== 'string' || !isHttpUrl(url)) return false
  await shell.openExternal(url)
  return true
})

ipcMain.on('amy:notify', (_event, payload: unknown) => {
...
ipcMain.on('amy:approval-set-visible', (_event, visible: unknown) => {
  if (visible !== true && visible !== false) return
  setApprovalVisible(visible)
})

ipcMain.on('amy:approval-set-size', (_event, height: unknown) => {
  if (approvalWindow === null || approvalWindow.isDestroyed()) return
