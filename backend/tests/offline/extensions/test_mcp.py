...
async def test_missing_environment_reference_fails_only_that_server(
    ...
    monkeypatch.delenv("AMY_MISSING_MCP_KEY", raising=False)
    config = MCPServerConfig(
        name="missing_env",
        command=sys.executable,
        args=("-c", "pass"),
        env={"API_KEY": "${AMY_MISSING_MCP_KEY}"},
    )
    registry = ToolRegistry()
    ...
    assert statuses[0].state is MCPServerState.FAILED
    assert "AMY_MISSING_MCP_KEY" in (statuses[0].error or "")
...
def test_mcp_environment_does_not_inherit_unlisted_host_secret(
    ...
    monkeypatch.setenv("AMY_HOST_SECRET", "must-not-leak")
    ...
    assert "AMY_HOST_SECRET" not in environment
