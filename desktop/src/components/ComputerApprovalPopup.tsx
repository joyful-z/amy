...
export function floatingApprovalPresentation(
  ...
  switch (phase) {
    ...
    case 'executing':
      return { eyebrow: '已允许', title: actionLabel, description: 'Amy 正在执行你刚刚允许的电脑操作。', status: '正在执行', tone: 'working' }
    case 'action_delivered':
      return { eyebrow: '操作已发送', title: actionLabel, description: '操作事件已经发送，Amy 正在确认界面是否真的发生变化。', status: '正在检查结果', tone: 'working' }
    case 'action_failed':
      return { eyebrow: '操作未完成', title: actionLabel, description: friendlyFloatingError(error, '这次电脑操作没有完成。'), status: 'Amy 正在判断是否可以恢复', tone: 'danger' }
    case 'continuing':
      return { eyebrow: '继续执行', title: 'Amy 正在继续任务', description: '刚才允许的操作已经结束，Amy 正在处理下一步。', status: '任务执行中', tone: 'working' }
    case 'run_completed':
      return { eyebrow: '任务完成', title: '本轮执行已完成', description: 'Amy 已经完成这次任务。', status: '已完成', tone: 'success' }
    case 'run_failed':
      return { eyebrow: '执行已停止', title: '本轮未能完成', description: friendlyFloatingError(error, 'Amy 无法继续本轮任务，你可以返回会话查看详情。'), status: '需要你的关注', tone: 'danger' }
    case 'denied':
...
function friendlyFloatingError(
  ...
  if (normalized.includes('maximum step') || normalized.includes('max_steps')) {
    return '执行步骤已达到上限。你可以返回会话，让 Amy 从现有结果继续。'
  }
