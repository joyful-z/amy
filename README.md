...
Amy 是一个面向长期工作的本地 AI Agent Harness。它不只完成当前对话，还会管理
...
```bash
git clone https://github.com/joyful-z/amy.git
cd amy
...
API 地址等非敏感配置保存在本地 `.amy` 目录；也可以继续使用 `backend/.env` 配置。
...
### 4. Start Amy Web for other devices

如果希望其他设备通过浏览器访问同一套 Amy UI，可以使用项目根目录下的 Web 启动脚本：

```bash
./Start\ Amy\ Web.command
```

脚本会将 Host 监听到 `0.0.0.0:8000`，并将 Vite Web UI 监听到
`0.0.0.0:5173`。同一局域网内的设备可以访问：

```text
http://<你的电脑局域网 IP>:5173
```

要让公网所有人访问，需要把 Host 和 Web UI 部署到公网服务器，或使用 Cloudflare Tunnel /
ngrok 之类的隧道把 `5173` 与 `8000` 暴露出去。公网开放会消耗你的模型 API Key，并可能
触发工具调用风险；正式对外前建议增加访问鉴权。
...
结果和即将执行的命令，再由用户确认安装。MCP 工具注册后仍会经过 Amy 的权限、执行、
