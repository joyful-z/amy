    ...
    <section className="extension-import">
      ...
      {plan ? <ImportPlanPreview plan={plan} /> : (
        <div className="extension-import__empty">
          ...
          <span>Amy 会识别 Skill 安装命令与 MCP Server，并展示实际动作。</span>
        </div>
      )}
  ...
  return (
    <div className="extension-import__preview">
      ...
      <div className="extension-import__actions-preview">
        ...
        {plan.requires_restart ? <p className="extension-import__restart">MCP 只写入配置，重启 Amy Host 后才会启动。</p> : null}
      </div>
    </div>
  )
}
...
function SkillList({
  ...
  return (
    <div className="extension-list-wrap">
      {skills.length === 0 ? (
        <EmptyState title="尚未安装 Skill" hint="添加一个方法包，让 Amy 在相关任务中按需激活。" icon="memory" />
      ) : (
  ...
  return (
    <div className="extension-list-wrap">
      <div className="mcp-config-note">
        ...
        <p>{restartRequired ? '配置已有变更等待应用，请重启 Amy Host。' : '新增 Server 会生成标准 JSON；保存后重启 Amy Host 才会连接并注册工具。'}</p>
      </div>
...
export function MCPInstallForm({
  ...
  return (
    <section className="extension-form">
      ...
      {(error || serverError) ? <div className="error-text extension-form__error">{error ?? String(serverError)}</div> : null}
      <div className="extension-form__notice">保存只写配置，不会在当前进程中执行命令。重启 Amy Host 后连接生效。</div>
